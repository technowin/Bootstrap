...
from hyper.settings import MEDIA_ROOT
app = Flask(__name__)
# Create your views here.
...
def preprocess_data_list(result_data, is_export):
    ...
    for row in result_data:
        processed_row = []
        for value in row:
            if isinstance(value, str) and 'hyperformfiles_' in value:
                file_ids = [v.replace('hyperformfiles_', '') for v in value.split(',') if v.startswith('hyperformfiles_')]
                uploaded_names = []
                if is_export == '1':
